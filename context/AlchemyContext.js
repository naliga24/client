...
export const AlchemyProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (account && chainId) {
        ...
        .then(async (balances) => {
          if (balances?.tokenBalances?.length) {
            ...
            for (let token of nonZeroBalances) {
              ...
              const fondToken = swapAvailableTokens?.find(
                (availableToken) =>
                  availableToken?.address?.trim().toLowerCase() ===
      ...
      //comment this because not support on Optimism, Arbitrum and more networks.
      // alchemy.ws.on(
      //   {
